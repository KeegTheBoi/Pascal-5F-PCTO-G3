...
namespace ServerGestioneMagazzino
{
    class ProtocolloComune
    {
        ...
        public void ConversioneMessaggio()
        {
            ...
            //Altrimenti viene segnalato al client dell'accaduto
        ...
        public bool VerificaOperazione(Operazione op)
        {
            ...
            if (Database.VerificaEsistenzaDato(queryOper))
            {
                ...
                string queryProd = $"SELECT IDCode FROM Magazzino WHERE IDCode = '{op.IDProdotto}';";
                if(!Database.VerificaEsistenzaDato(queryProd))
                {
                    _feedback += $"\nProdoto Inesistente\nInizializzione del processo di inserimento del Prodotto";
                    string inserProd = $"INSERT INTO Magazzino(IDCode, Descrizione, Quantità) VALUES ({op.IDProdotto}, '{op.NomeProdotto}', {op.QuantitaProdotti});";
                ...
                if(Database.VerificaEsistenzaDato(queryCarico))
                {
                    ...
                    if(carico)
                    {
                        ...
                        string queryAdd = $"UPDATE Magazzino SET Quantità = Quantità + {op.QuantitaProdotti} WHERE IDCode = {op.IDProdotto};";
                    ...
                    else
                    {
                        ...
                        string queryLeave = $"UPDATE Magazzino SET Quantità = Quantità - {op.QuantitaProdotti} WHERE IDCode = {op.IDProdotto};";
